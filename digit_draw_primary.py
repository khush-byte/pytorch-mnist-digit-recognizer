...
# Основной рабочий режим: распознавание разрешено, изменение весов,
# сброс модели и запись в датасеты полностью заблокированы.
PRIMARY_FROZEN_MODE = True
...
# ============================================================
# 4. ОСНОВНАЯ МОДЕЛЬ ЗАФИКСИРОВАНА
#
# MNIST replay не загружается, поскольку эта программа никогда
...
root.title(
    "Распознавание цифр — основная зафиксированная CNN"
)
...
def update_model_info():
    ...
    model_info_label.config(
        text=(
            f"Модель: {model_name}   |   "
            "Режим: зафиксирована, веса защищены   |   "
            f"Устройство: {device}"
        )
    )
...
def set_training_controls(enabled):
    ...
    correction_button.config(state=tk.DISABLED)
    train_all_button.config(state=tk.DISABLED)
    reset_model_button.config(state=tk.DISABLED)
    correct_digit_entry.config(state=tk.DISABLED)
...
def correct_and_train():
    ...
    if PRIMARY_FROZEN_MODE:
        correction_status.config(
            text="Основная модель зафиксирована: дообучение запрещено."
        )
        return
...
def reset_user_model():
    ...
    if PRIMARY_FROZEN_MODE:
        correction_status.config(
            text="Основная модель зафиксирована: сброс запрещён."
        )
        return
...
def save_test_sample():
    ...
    if PRIMARY_FROZEN_MODE:
        test_status.config(
            text=(
                "Запись в validation-набор отключена в основной программе."
            )
        )
        return
...
correction_instruction = tk.Label(
    right_frame,
    text=(
        "В этой рабочей версии веса модели зафиксированы. "
        "Дообучение, сброс и изменение датасетов отключены."
    ),
    font=(
        "Arial",
        11
...
test_instruction = tk.Label(
    right_frame,
    text=(
        "Для новых экспериментов используйте отдельные программы. "
        "Основное приложение работает только в режиме распознавания."
    ),
    font=(
        "Arial",
        10
...
save_test_button = tk.Button(
    test_input_frame,
    text="Запись в датасет отключена",
...
evaluate_test_button = tk.Button(
    right_frame,
    text="Оценка отключена в основном режиме",
