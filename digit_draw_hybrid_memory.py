...
# В гибридном режиме основной моделью всегда остаётся базовая CNN.
# Пользовательские исправления хранятся отдельно и не меняют веса.
HYBRID_MEMORY_MODE = True

# Похожие исправления используются только как подсказка. Автоматически
# заменить ответ CNN разрешено исключительно при точном совпадении.
MEMORY_AUTOMATIC_SIMILAR_OVERRIDE = False

# Пороги для отображения осторожной подсказки по похожему примеру.
MEMORY_MIN_EMBEDDING_SIMILARITY = 0.98
MEMORY_MIN_SHAPE_IOU = 0.30
MEMORY_MIN_COMBINED_SCORE = 0.88
MEMORY_MIN_LABEL_MARGIN = 0.02
...
# ============================================================
# 3A. ПАМЯТЬ ПОЛЬЗОВАТЕЛЬСКИХ ИСПРАВЛЕНИЙ
# ============================================================
...
def extract_memory_embeddings(images):

    # Берём 128 признаков перед последним классификационным слоем.
    features = model.features(images)
    features = model.classifier[0](features)
    features = model.classifier[1](features)
    features = model.classifier[2](features)
...
def reload_correction_memory():

    global memory_exact_labels
    global memory_embeddings
    global memory_masks
    global memory_labels
    global memory_paths
    global memory_sample_count

    # Если один рисунок исправлялся несколько раз, используем
    # последнюю по времени версию метки.
    newest_by_digest = {}
    ...
    for digit in range(10):
        ...
        for filename in os.listdir(directory):
            ...
            try:
                ...
                previous = newest_by_digest.get(digest)

                if previous is None or modified_time >= previous[0]:
                    newest_by_digest[digest] = (
                        modified_time,
                        path,
                        digit,
                        image,
                ...
                print(
                    "Пропускаю повреждённое исправление:",
                    path,
                    error,
                )

    records = sorted(
        newest_by_digest.items(),
    ...
    if not records:
        ...
        memory_paths = []
        memory_sample_count = 0
        print("Память исправлений пока пуста.")
        return
    ...
    for digest, (_, path, digit, image) in records:
        tensor = ToTensor()(image)
        images.append(tensor)
        labels.append(digit)
        paths.append(path)
        exact_labels[digest] = digit
    ...
    with torch.inference_mode():
        embeddings = extract_memory_embeddings(image_batch)

    memory_exact_labels = exact_labels
    memory_embeddings = embeddings
    memory_masks = image_batch > 0.20
    ...
    memory_paths = paths
    memory_sample_count = len(paths)

    print(
        "Исправлений загружено в память:",
        memory_sample_count,
    )


def find_correction_memory_match(image, image_tensor):

    digest = image_digest(image)

    if digest in memory_exact_labels:
        return {
            "label": memory_exact_labels[digest],
            "mode": "exact",
            "score": 1.0,
    ...
    if memory_embeddings is None or memory_sample_count == 0:
        return None

    query = image_tensor.to(device)
    ...
    with torch.inference_mode():
        query_embedding = extract_memory_embeddings(query)
    ...
    query_mask = query[0] > 0.20
    intersections = (
        memory_masks & query_mask
    ).sum(dim=(1, 2, 3)).float()
    unions = (
        memory_masks | query_mask
    ).sum(dim=(1, 2, 3)).float().clamp_min(1.0)
    shape_scores = intersections / unions

    combined_scores = (
        0.85 * embedding_scores
        + 0.15 * shape_scores
    )

    best_index = combined_scores.argmax().item()
    best_label = memory_labels[best_index].item()
    best_score = combined_scores[best_index].item()
    best_embedding = embedding_scores[best_index].item()
    best_shape = shape_scores[best_index].item()

    other_labels = memory_labels != best_label

    if other_labels.any():
        second_score = combined_scores[other_labels].max().item()
    else:
        second_score = -1.0
    ...
    if (
        best_embedding < MEMORY_MIN_EMBEDDING_SIMILARITY
        or best_shape < MEMORY_MIN_SHAPE_IOU
        or best_score < MEMORY_MIN_COMBINED_SCORE
        or margin < MEMORY_MIN_LABEL_MARGIN
    ):
        return None
    ...
    return {
        ...
        "mode": "similar",
        "score": best_score,
        "embedding_similarity": best_embedding,
        "shape_iou": best_shape,
        "margin": margin,
        "path": memory_paths[best_index],
    }


reload_correction_memory()


# ============================================================
# 4. ОНЛАЙН-ДООБУЧЕНИЕ ОТКЛЮЧЕНО
#
# Базовая CNN защищена от изменения. Переменная оставлена только
# для совместимости со старыми функциями, которые недоступны в UI.
# ============================================================
...
root.title(
    "Распознавание цифр — базовая CNN с памятью исправлений"
)
...
def update_model_info():
    ...
    model_info_label.config(
        text=(
            f"Основная модель: {BASE_MODEL_FILE} (защищена)   |   "
            f"Файлов исправлений: {corrections}   |   "
            f"Уникальных примеров в памяти: {memory_sample_count}   |   "
            f"Устройство: {device}"
        )
    )
...
def set_training_controls(enabled):
    ...
    train_all_button.config(state=tk.DISABLED)
    reset_model_button.config(state=tk.DISABLED)
    correct_digit_entry.config(state=state)
    save_test_button.config(state=tk.DISABLED)
    evaluate_test_button.config(state=tk.DISABLED)
    open_errors_button.config(state=tk.DISABLED)
    test_digit_entry.config(state=tk.DISABLED)
...
def predict():
    ...
    base_prediction = probabilities.argmax(
        dim=1
    ).item()

    base_confidence = probabilities[
        0,
        base_prediction,
    ].item() * 100

    memory_match = find_correction_memory_match(
        mnist_image,
        tensor,
    )
    ...
    if (
        ...
        and (
            memory_match["mode"] == "exact"
            or MEMORY_AUTOMATIC_SIMILAR_OVERRIDE
        )
        and memory_match["label"] != base_prediction
    ):
        final_prediction = memory_match["label"]
        last_prediction = final_prediction

        result_label.config(
            text=(
                f"AI + память считает, что это: "
                f"{final_prediction}"
            )
        )

        memory_description = "точное сохранённое исправление"

        confidence_label.config(
            text=(
                f"Источник: {memory_description}"
            )
        )

        probability_label.config(
            text=(
                f"Базовая CNN: {base_prediction} "
                f"({base_confidence:.2f}%)\n\n"
                + format_probabilities(probabilities)
            )
        )

        correction_status.config(
            text=(
                f"Память заменила ответ базовой CNN "
                f"{base_prediction} на {final_prediction}. "
                "Веса нейросети не изменялись."
            )
        )

        return

    predicted_digit, confidence = show_prediction(
        probabilities
    )

    if (
        memory_match is not None
        and memory_match["label"] == predicted_digit
    ):
        correction_status.config(
            text=(
                f"Базовая CNN и память согласны: {predicted_digit}. "
                "Если ответ неверный — укажите правильную цифру."
            )
        )
    elif memory_match is not None:
        correction_status.config(
            text=(
                f"Ответ базовой CNN: {predicted_digit}. "
                f"Память только предлагает {memory_match['label']} "
                f"(сходство {memory_match['score'] * 100:.2f}%), "
                "но не заменяет ответ автоматически."
            )
        )
    else:
        correction_status.config(
            text=(
                f"Ответ базовой CNN: {predicted_digit}. "
                "Если он неверный — сохраните правильный ответ в память."
            )
        )
...
def remember_correction():
    ...
    if last_mnist_image is None:
        correction_status.config(
            text=(
                "Сначала нарисуйте цифру и нажмите «Распознать»."
            )
        )
        return

    value = correct_digit_var.get().strip()
    ...
    correct_digit = int(value)

    if last_prediction == correct_digit:
        correction_status.config(
            text=(
                "Текущий итоговый ответ уже правильный. "
                "Новое исправление не требуется."
            )
        )
        return
    ...
    reload_correction_memory()
    last_prediction = correct_digit

    result_label.config(
        text=(
            f"Исправление запомнено: {correct_digit}"
        )
    )

    confidence_label.config(
        text="Источник: точное пользовательское исправление"
    )

    correction_status.config(
        text=(
            f"✓ Сохранено в память. При повторении этого рисунка "
            f"ответ будет {correct_digit}. Базовая CNN не изменена."
        )
    )
    ...
    print()
    print("Исправление добавлено в память:", path)
    print("Правильная цифра:", correct_digit)
    print("Базовая модель не изменялась:", BASE_MODEL_FILE)
...
correction_instruction = tk.Label(
    right_frame,
    text=(
        "Если AI явно ошибся, укажите правильную цифру и сохраните "
        "её в память. Автоматически применяется только точное "
        "совпадение; похожее показывается как подсказка."
    ),
    font=(
        "Arial",
        11
...
correction_button = tk.Button(
    right_frame,
    text="Запомнить исправление",
    command=remember_correction,
    font=(
        "Arial",
        12,
        "bold"
...
train_all_button = tk.Button(
    right_frame,
    text="Онлайн-дообучение отключено",
...
test_title = tk.Label(
    right_frame,
    text="Оценка перенесена в отдельную программу",
    font=(
        "Arial",
        14,
        "bold"
...
test_instruction = tk.Label(
    right_frame,
    text=(
        "Для независимой оценки используйте digit_draw_final_test.py. "
        "Validation и final test здесь защищены от изменения."
    ),
    font=(
        "Arial",
        10
...
save_test_button = tk.Button(
    test_input_frame,
    text="Сохранение теста отключено",
...
evaluate_test_button = tk.Button(
    right_frame,
    text="Откройте digit_draw_final_test.py",
